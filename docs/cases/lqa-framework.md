...
### Оптимизация архитектуры: Миграция на микросервисы (FastAPI / Docker)

При масштабировании нагрузок было выявлено узкое место: подсистема технической валидации пунктуации, основанная на тяжелых каскадных регулярных выражениях под 37 языковых специфик, создавала избыточную нагрузку на ядро монолитного пайплайна.

Для снижения технического долга и обеспечения принципа слабой связанности (Loose Coupling) был проведен успешный рефакторинг:
* **Выделение сервиса:** Логика проверки лингвистических знаков и регулярных выражений была полностью изолирована и извлечена из монолитного ядра.
* **Проектирование API-контракта:** Модуль оформлен как самостоятельное веб-приложение на базе фреймворка **FastAPI**, принимающее сырой текст по REST API и возвращающее валидированный структурированный JSON-пакет данных.
* **Контейнеризация:** Для стандартизации окружения и независимого деплоя приложение было контейнеризировано с помощью **Docker** (использован оптимизированный образ `python-slim`).

**Результат:** Тяжелые регулярные выражения переведены на изолированный микросервисный цикл. Это разгрузило центральный конвейер, предотвратило коллизии зависимостей библиотек и обеспечило архитектурную готовность системы к горизонтальному масштабированию.

## Стек и отчетность

### Технологический стек

* **Язык и манипуляция данными:** Python, Pandas, Numpy.
* **Архитектура и инфраструктура:** FastAPI, Docker, REST API.
