...
```mermaid
graph TD
    A["[Extract] Входящие форматы:<br>xlsx, xliff, properties"] --> B["[Transform 1] Техническая валидация"]
    B --> C["[Transform 2] NLP-препроцессинг<br>и лемматизация"]
    C --> D["[Transform 3] Синтаксический анализ<br>и проверка глоссария"]
    D --> E["[Transform 4] Семантический<br>векторный анализ (LaBSE)"]
    E --> F["[Load] Генерация диффов и<br>структурированного отчета"]
