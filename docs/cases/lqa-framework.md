...
```mermaid
graph TD
    subgraph E [Extract — Сбор и извлечение]
        A["Входящие форматы:<br>xlsx, xliff, properties"]
    end
    
    subgraph T [Transform — Машинная очистка и NLP-анализ]
        B["Техническая валидация"] --> C["NLP-препроцессинг<br>и лемматизация"]
        C --> D["Синтаксический анализ<br>и проверка глоссария"]
        D --> E1["Семантический<br>векторный анализ (LaBSE)"]
    end
    
    subgraph L [Load — Генерация и загрузка]
        E1 --> F["Генерация диффов и<br>структурированного отчета"]
    end
